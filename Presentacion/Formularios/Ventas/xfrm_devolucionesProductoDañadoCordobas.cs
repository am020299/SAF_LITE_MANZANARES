...
{
    public partial class xfrm_devolucionesProductoDañadoCordobas : DevExpress.XtraEditors.XtraForm
    {
        public xfrm_devolucionesProductoDañadoCordobas()
        {
            InitializeComponent();
        }
        ...
        {
            srch_facturas.Properties.DataSource = Negocio.ClasesCN.VentasCN.Ventas_SELECT_DevolucionesProductoDañado(1).Where(o => o.activo == true && o.estado == 2 && o.moneda == 1 && o.dias == 0);
            ...
            if (Validar())
            {
                ...
                if(gridViewDetalleModSalida.RowCount > 0)
                {
                    for (int i = 0; i < gridViewDetalleModSalida.RowCount; i++)
                    {
                        ...
                        var queryKardex = Negocio.ClasesCN.InventarioCN.Kardex_SelectVentaEXC().Where(x => x.id_bodega == 4 && int.Parse(x.ubicacion) == 0 && x.moneda == 1 && x.id == id_producto).FirstOrDefault();
        ...
        private void repositoryItemSearchLookUpEditProductoModSalida_EditValueChanged(object sender, EventArgs e)
        {
            ...
            var queryPrecio = Negocio.ClasesCN.InventarioCN.Kardex_SelectVentaEXC_Devolucion(id_venta).Where(x => x.id_bodega == 4 && int.Parse(x.ubicacion) == 0 && x.moneda == 1 && x.id == id_producto).FirstOrDefault();
        ...
        private void srch_facturas_EditValueChanged(object sender, EventArgs e)
        {
            ...
            repositoryItemSearchLookUpEditProductoModSalida.DataSource = Negocio.ClasesCN.InventarioCN.Kardex_SelectVentaEXC_Devolucion(id_venta).Where(x => x.id_bodega == 4 && int.Parse(x.ubicacion) == 0 && x.moneda == 1).ToList();
        ...
        void GuardarDevolucion()
        {
            ...
            Clases.MyMessageBox.MessageFont = Clases.MyMessageBox.MessageFont = new Font(Clases.MyMessageBox.MessageFont.FontFamily, 18, FontStyle.Bold);
            if (Clases.MyMessageBox.Show($"¿Esta completamente seguro de guardar esta devolución de productos fallado del cliente?", "Informacion", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;

            string numeroDocumento = Negocio.ClasesCN.InventarioCN.RetornarNumeroDocumentoProductoFallado();
            var queryDevolucionEntrada = Negocio.ClasesCN.InventarioCN.DevolucionEntradaProdDañado(id_venta, observaciones, id_empleado, persona_referencia, gridViewDetalleMod, numeroDocumento);
            var queryDevolucionSalida = Negocio.ClasesCN.InventarioCN.DevolucionSalidaProdDañado(id_venta, observaciones, id_empleado, persona_referencia, gridViewDetalleModSalida, numeroDocumento);
            var queryDevolucionCambio = Negocio.ClasesCN.InventarioCN.SaldoFactura(id_venta, observaciones, id_empleado, persona_referencia, gridViewDetalleMod, gridViewDetalleModSalida, 2);
            if (queryDevolucionEntrada > 0)
            {
                Clases.MyMessageBox.MessageFont = Clases.MyMessageBox.MessageFont = new Font(Clases.MyMessageBox.MessageFont.FontFamily, 18, FontStyle.Bold);
                Clases.MyMessageBox.Show($"Devolucion de Producto Fallado del Cliente. Realizado correctamente.", "Informacion", MessageBoxButtons.OK, MessageBoxIcon.Information);
